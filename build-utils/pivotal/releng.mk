...
GREP_SED_VAR = $(BLD_TOP)/build-utils/pivotal/make/dependencies/ivy.xml | sed -e 's|\(.*\)rev="\(.*\)"[ ]*conf\(.*\)|\2|'
...
sync_tools: opt_write_test /opt/releng/apache-ant
	rm -rf $(BLD_TOP)/ext
	@cd $(BLD_TOP)/build-utils/pivotal/make/dependencies; \
	 (umask 002; ANT_HOME=/opt/releng/apache-ant/ ANT_OPTS="-Djavax.net.ssl.trustStore=$(BLD_TOP)/build-utils/pivotal/make/dependencies/cacerts" /opt/releng/apache-ant/bin/ant -DBLD_ARCH=$(BLD_ARCH) resolve);
