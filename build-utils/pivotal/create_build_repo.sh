...
##
## Retrieve gpdemo for running ICG post-build
##
pushd ${BLDWRAP_TOP}/src
rm -rf gpdemo
git clone ${GPDEMO_GIT_URL}
popd

##
## Retrieve pxf_tools for creating single-cluster
##
pushd ${BLDWRAP_TOP}/src/pxf
rm -rf tools
git clone ${PXF_TOOLS_GIT_URL} tools
popd
...
git clone -b hawq2.0 ${RELENG_BUILD_URL} /tmp/releng-build
RETURN=$?
if [ ${RETURN} != 0 ]; then
